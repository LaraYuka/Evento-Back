...
class UserController
{
    ...
    public function gravaUser()
    {
        ...
        if (!isset($objJson->senha) || strlen($objJson->senha) < 4 || !preg_match('/^\d+$/', $objJson->senha)) {
            http_response_code(400);
            die(json_encode(["error" => "A senha deve conter apenas números e ter pelo menos 4 caracteres."]));
        }
        $user = new User();
    ...
    public function atualizaUser(User $user)
    {
        ...
        if ($userById->id != $user->id) {
            ...
            die("Não é possivel atualizar outro usuário que não seja você mesmo.");
        }
    ...
    public function alterarSenha(User $user)
    {
        ...
        if (!isset($objJson->nova_senha) || !isset($objJson->confirmacao_senha)) {
            http_response_code(400);
            die(json_encode(["error" => "Nova senha e confirmação são obrigatórias."]));
        }

        if ($objJson->nova_senha !== $objJson->confirmacao_senha) {
            http_response_code(400);
            die(json_encode(["error" => "As senhas não coincidem."]));
        }

        if (!preg_match('/^\d{4,}$/', $objJson->nova_senha)) {
            http_response_code(400);
            die(json_encode(["error" => "A senha deve conter apenas números e ter pelo menos 4 caracteres."]));
        }

        // Atualiza a senha
        $user->senha = password_hash($objJson->nova_senha, PASSWORD_DEFAULT);
        if ($user->update()) {
            http_response_code(200);
            echo json_encode(["message" => "Senha alterada com sucesso."]);
        } else {
            http_response_code(400);
            die(json_encode(["error" => "Erro ao alterar a senha."]));
        }
    }
}
